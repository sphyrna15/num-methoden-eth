...
from numpy import double, sign, finfo, array, zeros, dot, mod, size, inf, all, max, min, abs
...
def ode45(vfun, vslot, vinit, **kwargs):
    """
    ode45(vfun, vslot, vinit, **kwargs)
        Solves a differential equation.

        Return the timesteps and time evolution for a differential equation
        y' = f(t,y) (or M(t,y) * y' = f(t,y) if a mass matrix is specified).
        ...
        vslot : list
           ...
           The solver imposes the initial conditions at vslot[0],
           and integrates from vslot[0] to vslot[-1]. To obtain solutions at
           specific times (all increasing or all decreasing),
           use vslot = [t0,t1,...,tf].
        vinit : ndarray
           A vector of initial conditions
        **kwargs : optional parameters
           ``normcontrol`` : 'on' or 'off'
              Set this property on to request that the solvers control the
              error in each integration step with
              norm(e) <= max(RelTol*norm(y),AbsTol).
              By default the solvers use a more stringent componentwise
              error control.


            ``nonnegative`` : list of integers
              Specifies which components of the solution
              vector must be nonnegative. The default value is [].
              nonnegative is not available for problems where there
              is a mass matrix.


            ``reltol`` : positive float or ndarray
              This tolerance is a measure of the error relative
              to the size of each solution component. Roughly, it controls
              the number of correct digits in all solution components, except
              those smaller than thresholds AbsTol.
              Can be array of same dimension as vslot, such that
              relative error tolerances apply to the individual components
              of the solution vector.
              Default is 1e-5.


            ``abstol`` : positive float or ndarray
              abstol is a threshold below which the value of each
              solution component is unimportant. The absolute error
              tolerances determine the accuracy when the solution
              approaches zero.
              Can be array of same dimension as vslot, such that
              relative error tolerances apply to the individual components
              of the solution vector.
            ...
            ``outputsel`` : list of integers
              Specifies which components of the solution vector
              are to be passed to the output function / which components
              to save.


            ``outputsave`` : integer
              Save output every outputsave step.


            ``initialstep``: positive float
              Suggested initial step size.
              InitialStep sets an upper bound on the magnitude of the first
              step size the solver tries. If you do not set InitialStep,
              the initial step size is based on the slope of the solution at
              the initial time vslot[0], and if the slope of all solution
              components is zero, the procedure might try a step size that
              is much too large. If you know this is happening or you want to
              be sure that the solver resolves important behavior at the start
              of the integration, help the code start by providing a suitable
              InitialStep.


            ``maxstep``: positive float
              Upper bound on solver step size.
              If the differential equation has periodic coefficients or
              solutions, it might be a good idea to set MaxStep to some fraction
              (such as 1/4) of the period. This guarantees that the solver
              does not enlarge the time step too much and step over a period
              of interest.


            ``mass``: callable ``M(t)``
              For problems of the form M(t)*y'=f(t,y), set 'mass' to a mass
              matrix M. For problems of the form M(t,y)*y'=f(t,y), set 'Mass' to
              a function handle Mfun, where Mfun(t,y) evaluates the mass
              matrix M(t,y).


            ``stats``: 'on' or 'off'
              By default, Stats is off. If it is on, after solving the problem
              the solver displays
